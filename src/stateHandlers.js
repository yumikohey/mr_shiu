...
var NUMBER_OF_AUDIOS = 5;
var host = 'http://meme.ipkpnmempp.us-west-2.elasticbeanstalk.com';
...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            this.attributes['playOrder'] = Array.apply(null, {length: NUMBER_OF_AUDIOS}).map(Number.call, Number);
            ...
            var message = 'Welcome to Jay Sun Shiu Sick Podcast. You can say, play the audio to begin the podcast.';
            var reprompt = 'You can say, play the audio, to begin.';
            var _this = this;
            var completePath = host + "/alexas/last_five_videos";
            axios.get(completePath)
                .then(function(response){
                    var result = response.data;
                    _this.attributes['audio_data'] = result;
        ...
        'Unhandled' : function () {
            this.handler.state = constants.states.START_MODE;
            var message = 'You can say, play the audio, to begin the podcast.';
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                this.handler.state = constants.states.START_MODE;
                message = 'Welcome to Jay Sun Shiu Sick Podcast. You can say, play the audio to begin the podcast.';
                reprompt = 'You can say, play the audio, to begin.';
            } else {
                this.handler.state = constants.states.RESUME_DECISION_MODE;
                var currentIndex = this.attributes['index'] + 1;
                message = 'You were listening to number ' + currentIndex +
                    ' audio. Would you like to resume?';
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
        ...
        'LaunchRequest' : function () {
            var currentIndex = this.attributes['index'] + 1;
            var message = 'You were listening to number ' + currentIndex +
                    ' audio. Would you like to resume?';
...
var controller = function () {
    return {
        play: function () {
            ...
            if (this.attributes["audio_data"]) {
                var podcast = this.attributes["audio_data"][this.attributes['playOrder'][this.attributes['index']]];
...
function shuffleOrder(callback) {
    ...
    var array = Array.apply(null, {length: NUMBER_OF_AUDIOS}).map(Number.call, Number);
